...
func (r *ViewableRepo) IncrementView(userID, viewableID uint) error {
	if err := r.SaveView(userID, viewableID); err != nil {
		return err
	}
...
func (r *ViewableRepo) SaveView(userID, viewableID uint) error {
	var count int

	r.db.Raw("SELECT 1 FROM views WHERE user_id = ? AND viewable_id = ?", userID, viewableID).Count(&count)
	if count < 1 {
		r.db.Exec("INSERT INTO views(user_id, viewable_id) VALUES (?, ?)", userID, viewableID)
	}
